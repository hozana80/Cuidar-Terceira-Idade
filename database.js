...
const criarBanco = async () => {
  ...
  const todosatendimentos = await db.all(
    `SELECT * FROM atendimentos`);
  console.table(todosatendimentos);

  // Exemplo de SELECT específico
  const atendimentosDonaMaria = await db.all(`SELECT * FROM atendimentos WHERE nome_paciente = "Dona Maria"`);
  console.table(atendimentosDonaMaria);
